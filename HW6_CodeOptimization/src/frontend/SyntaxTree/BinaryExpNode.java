...
public class BinaryExpNode extends ExpNode {
            ...
            switch (binaryOp.getTypeCode()) {
                ...
                case MINU:
                    if (left.getValue() == 0) {
                        ...
                        BinaryExpNode right = (BinaryExpNode) rightExp;
                        if ((right.binaryOp.getTypeCode() == PLUS || right.binaryOp.getTypeCode() == MINU) && right.leftExp instanceof NumberNode) {
                            return new BinaryExpNode(symbolTable, new Token(right.binaryOp.isType(PLUS) ? MINU : PLUS, 0),
                                    new NumberNode(left.getValue() - ((NumberNode) right.leftExp).getValue()), right.rightExp).simplify();
                        }
                    }
                    return new BinaryExpNode(symbolTable, binaryOp, leftExp, rightExp);
                case MULT:
    ...
    @Override
    public Type getType() {
        if (binaryOp.isType(PLUS) || binaryOp.isType(MINU) || binaryOp.isType(MULT) || binaryOp.isType(DIV) || binaryOp.isType(MOD)) {
            return INT;
        } else {
            return BOOL;
        }
    ...
    public Value generateMidCode() {
        ...
        midCodeTable.addMidCode(new Assign(true, value,
                new BinaryOperate(map.get(binaryOp.getTypeCode()), leftValue, rightValue)));
